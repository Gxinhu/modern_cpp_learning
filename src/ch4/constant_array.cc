// array is diffident with vector, the former one is constant, and later one is dynamic
...
void learn_array() {
    ...
    fmt::print("Arrays: {}\n", arrays);
    std::sort(arrays.begin(), arrays.end(), [](int x, int y) {
        return x > y;
    });
    std::sort(&arr1[0], &arr1[4], [](int x, int y) {
        return x > y;
    });
